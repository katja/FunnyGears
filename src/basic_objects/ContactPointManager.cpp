    ...
    do {
        ...
        if(occurencesCP + occurencesNCP == 0) {
            ...
            if(it.isCurrentlyInCorrectCP()) {
                ...
                std::cerr << "DEFECT: found point with NOT CORRECT NCP AND NOT CORRECT CP in ContactPointManager!\nThis should not happen!" << std::endl;
...
real ContactPointManager::contactPositionCoverageAngle(vector<ContactPoint*> &contactPoints, ContactPoint *&start, ContactPoint *&end) const {
    ...
    ContactPoint *lastPoint = contactPoints.front();
    ...
    for(ContactPoint *cp : contactPoints) {
        if(cp != firstPoint) {
            if((listDirection == -1 && (cp->turningAngleToContact > firstPoint->turningAngleToContact)) ||
               (listDirection == +1 && (cp->turningAngleToContact < firstPoint->turningAngleToContact)))
                firstPoint = cp;
        }
        if(cp != lastPoint) {
            if((listDirection == -1 && (cp->turningAngleToContact < lastPoint->turningAngleToContact)) ||
               (listDirection == +1 && (cp->turningAngleToContact > lastPoint->turningAngleToContact)))
                lastPoint = cp;
        }
    }
    start = firstPoint;
    ...
    return absolute(lastPoint->turningAngleToContact - firstPoint->turningAngleToContact);
}
